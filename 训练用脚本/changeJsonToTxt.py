...
def convert_labelme_to_yolov5(json_path, image_width, image_height):
    ...
    for shape in shapes:
        ...
        y_center = (bbox[0][1] + bbox[0][0]) / 2
        width = abs(bbox[1][0] - bbox[0][0])
        height = abs(bbox[0][1] - bbox[0][0])
...
# 处理指定文件夹内的所有JSON文件
json_folder = 'D:/learn/trywork/python/HOK/train2'  # 替换为包含JSON文件的文件夹路径
image_folder = 'D:/learn/trywork/python/HOK/train2'  # 替换为与JSON文件对应的图像文件夹路径
output_folder = 'D:/learn/trywork/python/HOK/train2'  # 替换为要保存TXT文件的文件夹路径
